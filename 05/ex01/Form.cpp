...
/*   Updated: 2020/10/11 17:06:02 by akramp        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
std::ostream & operator << (std::ostream &out, Form &obj)
{
	if (obj.getSigned() == true)
		out << "The " << obj.getName() << " form with " << obj.getGradeSign()
			<< " as grade to sign, " << obj.getGradeExe()
			<< " as grade to execute and the form is signed!";
	else
		out << "The " << obj.getName() << " form with " << obj.getGradeSign()
			<< " as grade to sign, " << obj.getGradeExe()
			<< " as grade to execute and the form is not signed.";
	return (out);
}
